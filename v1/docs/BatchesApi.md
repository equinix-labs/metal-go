...
func main() {
    ...
    resp, r, err := api_client.BatchesApi.CreateDeviceBatch(context.Background(), id).Batch(batch).Execute()
    if err != nil {
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    api_client := openapiclient.NewAPIClient(configuration)
    resp, r, err := api_client.BatchesApi.FindBatchesByProject(context.Background(), id).Include(include).Exclude(exclude).Execute()
    if err != nil {
