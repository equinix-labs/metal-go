...
func Command(ctx context.Context, client ClientService) *cobra.Command {
	command := cobra.Command{
		Use:   "regions",
		Short: "Client for regions",
	}

	command.AddCommand(findRegions(ctx, client))

	return &command
}

func findRegions(ctx context.Context, client ClientService) *cobra.Command {
	parameters := &FindRegionsParams{}

	command := &cobra.Command{
		Use: "FindRegions",
		RunE: func(cmd *cobra.Command, args []string) error {

			result, err := client.FindRegions(parameters, nil)
			if err != nil {
				return err
			}
